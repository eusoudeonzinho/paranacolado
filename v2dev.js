(function() {
    ...
    function showCustomAlert(message, type = 'info') {
        ...
        const overlay = document.createElement('div'); overlay.id = 'bmAlertOverlay';
        const alertBox = document.createElement('div'); alertBox.id = 'bmAlertBox'; alertBox.classList.add(`bmAlert-${type}`);
        const messageP = document.createElement('p'); messageP.id = 'bmAlertMessage'; messageP.textContent = message;
        const closeBtn = document.createElement('button'); closeBtn.id = 'bmAlertCloseBtn'; closeBtn.textContent = 'OK';
        closeBtn.onclick = () => {
            alertBox.classList.add('bmAlertFadeOut'); overlay.style.opacity = 0;
            setTimeout(() => { if (document.body.contains(overlay)) { document.body.removeChild(overlay); } }, 300);
        };
        alertBox.appendChild(messageP); alertBox.appendChild(closeBtn); overlay.appendChild(alertBox); document.body.appendChild(overlay);
        void alertBox.offsetWidth; alertBox.classList.add('bmAlertPopIn'); overlay.style.opacity = 1;
    }
    ...
    function dispatchKeyEvent(target, eventType, key, keyCode, charCode = 0) { /* ... código mantido ... */
        let effectiveCharCode = charCode; if (!effectiveCharCode && key && key.length === 1) { effectiveCharCode = key.charCodeAt(0); }
        const event = new KeyboardEvent(eventType, { key: key, code: `Key${key.toUpperCase()}`, keyCode: keyCode, which: keyCode, charCode: eventType === 'keypress' ? effectiveCharCode : 0, bubbles: true, cancelable: true });
        target.dispatchEvent(event);
    }
    async function simulateBackspace(targetElement) { /* ... código mantido ... */
        if (!targetElement) return; activeEl = targetElement; targetElement.focus(); const start = targetElement.selectionStart; const end = targetElement.selectionEnd;
        dispatchKeyEvent(targetElement, 'keydown', 'Backspace', 8);
        if ((targetElement.tagName === 'INPUT' || targetElement.tagName === 'TEXTAREA')) {
            const currentValue = targetElement.value; let newValue = currentValue; let newCursorPos = start;
            if (start === end && start > 0) { newValue = currentValue.substring(0, start - 1) + currentValue.substring(end); newCursorPos = start - 1; }
            else if (start !== end) { newValue = currentValue.substring(0, start) + currentValue.substring(end); newCursorPos = start; }
            if (newValue !== currentValue) {
                const prototype = Object.getPrototypeOf(targetElement); const descriptor = Object.getOwnPropertyDescriptor(prototype, 'value');
                if (descriptor && descriptor.set) { descriptor.set.call(targetElement, newValue); } else { targetElement.value = newValue; }
                targetElement.selectionStart = targetElement.selectionEnd = newCursorPos;
                targetElement.dispatchEvent(new Event('input', { bubbles: true, cancelable: true })); targetElement.dispatchEvent(new Event('change', { bubbles: true, cancelable: true }));
            }
        } else if (targetElement.isContentEditable) { document.execCommand('delete', false, null); }
        dispatchKeyEvent(targetElement, 'keyup', 'Backspace', 8); await new Promise(r => setTimeout(r, 1));
    }
    function sendChar(c) { /* ... código mantido ... */
        if (!activeEl) { console.warn("sendChar chamada, mas activeEl é nulo."); showCustomAlert("Erro: Clique no campo onde deseja digitar antes.", 'error'); return; }
        if (!document.body.contains(activeEl)) { console.warn("sendChar chamada, mas activeEl não está mais no DOM."); showCustomAlert("Erro: O campo selecionado não existe mais.", 'error'); return; }
        const targetElement = activeEl; targetElement.focus(); const keyCode = c.charCodeAt(0);
        dispatchKeyEvent(targetElement, 'keydown', c, keyCode); dispatchKeyEvent(targetElement, 'keypress', c, keyCode, keyCode);
        if (targetElement.isContentEditable) { document.execCommand('insertText', false, c); }
        else if (targetElement.tagName === 'INPUT' || targetElement.tagName === 'TEXTAREA') {
            const start = targetElement.selectionStart; const end = targetElement.selectionEnd; const currentValue = targetElement.value; const newValue = currentValue.substring(0, start) + c + currentValue.substring(end);
            const prototype = Object.getPrototypeOf(targetElement); const descriptor = Object.getOwnPropertyDescriptor(prototype, 'value');
            if (descriptor && descriptor.set) { descriptor.set.call(targetElement, newValue); } else { targetElement.value = newValue; }
    ...
    splash.innerHTML = `
        <div id="bmSplashContent">
             <img id="bmSplashImg" src="https://i.imgur.com/RUWcJ6e.png"/> <div id="bmSplashTitle">Paraná Tools</div>
             <div id="bmSplashSubtitle">Carregando...</div> <div id="bmLoadingBar"><div id="bmLoadingProgress"></div></div>
        </div> <div id="bmSplashBgEffect"></div>
    `;
    ...
    setTimeout(() => {
        ...
        // Lógica de arrastar
        const header = document.getElementById('bmHeader'); let isDragging = false; let dragStartX, dragStartY, initialLeft, initialTop;
        ...
        function onMouseMove(e) { if (!isDragging) return; const dx = e.clientX - dragStartX; const dy = e.clientY - dragStartY; wrapper.style.left = initialLeft + dx + 'px'; wrapper.style.top = initialTop + dy + 'px'; }
        function onMouseUp() { if (isDragging) { isDragging = false; header.style.cursor = 'move'; document.removeEventListener('mousemove', onMouseMove); document.removeEventListener('mouseup', onMouseUp); } }

        // =============================================================
        // Lógica “Modo Disfarçado” (CORRIGIDA + Verificação de 'ev')
        // =============================================================
        const toggleWrapper = document.getElementById('bmToggleWrapper'); const toggleBox = document.getElementById('bmToggleImg');
        let stealthOn = false; let firstTimeStealth = true; let rect = null;

        function handleStealthMouseMove(ev) { // 'ev' é o objeto do evento
            // ---> INÍCIO DA VERIFICAÇÃO ADICIONADA <---
            // Verifica se 'ev' existe e tem as propriedades necessárias. Se não, sai da função.
            if (!ev || typeof ev.clientX === 'undefined' || typeof ev.clientY === 'undefined') {
                console.warn("handleStealthMouseMove chamada sem um objeto de evento válido.", ev);
                return;
            }
            // ---> FIM DA VERIFICAÇÃO ADICIONADA <---

            if (!stealthOn || !wrapper || !document.body.contains(wrapper)) { exitStealth(); return; }
            if (!rect) { rect = wrapper.getBoundingClientRect(); if (!rect || rect.width === 0 || rect.height === 0) return; }
            ...
            if (isInside) {
                ...
                if (wrapper.style.opacity !== '0') {
                    rect = wrapper.getBoundingClientRect(); // Atualiza ANTES de esconder
                    if (rect && rect.width > 0 && rect.height > 0) {
                        wrapper.style.opacity = 0; wrapper.style.pointerEvents = 'none';
                    } else { console.warn("Rect inválido antes de esconder."); }
                }
            }
        }
        ...
        function enterStealth() {
            ...
            stealthOn = true; wrapper.classList.add('stealth-mode'); toggleBox.classList.add('active');
            wrapper.style.opacity = 1; wrapper.style.pointerEvents = 'auto'; rect = wrapper.getBoundingClientRect();
            if (!rect || rect.width === 0 || rect.height === 0) {
                 console.error("Falha ao obter rect em enterStealth."); stealthOn = false; wrapper.classList.remove('stealth-mode'); toggleBox.classList.remove('active');
                 showCustomAlert("Erro ao ativar Modo Disfarçado.", "error"); return;
            }
            document.addEventListener('mousemove', handleStealthMouseMove); // Adiciona listener
            wrapper.style.opacity = 0; wrapper.style.pointerEvents = 'none'; // Esconde
            console.log('Entered Stealth Mode');
        }

        function exitStealth() {
            stealthOn = false; document.removeEventListener('mousemove', handleStealthMouseMove); // Remove listener
            if (wrapper && document.body.contains(wrapper)) {
                 wrapper.classList.remove('stealth-mode'); toggleBox.classList.remove('active');
                 wrapper.style.opacity = 1; wrapper.style.pointerEvents = 'auto';
             }
            rect = null; console.log('Exited Stealth Mode');
        }

        function showStealthOverlay() { /* ... código mantido ... */
            const ov = document.createElement('div'); ov.id = 'bmOv';
            ov.innerHTML = `<div id="bmOvContent"><img src="https://i.imgur.com/RquEok4.gif" alt="Demo"/> <p>O Modo Disfarçado oculta esta janela...</p> <button id="bmOvBtn">Entendido</button></div>`;
            document.body.appendChild(ov);
            document.getElementById('bmOvBtn').onclick = () => {
                ov.style.opacity = 0; setTimeout(() => { if (document.body.contains(ov)){ ov.remove(); } }, 500);
                enterStealth(); // Chama enterStealth APÓS fechar
            };
        }

        toggleWrapper.onclick = () => { if (!stealthOn) { if (firstTimeStealth) { firstTimeStealth = false; showStealthOverlay(); } else { enterStealth(); } } else { exitStealth(); } };
        // =============================================================
        // Fim da Lógica “Modo Disfarçado”
        // =============================================================
        ...
        startButton.onclick = async function() { /* ... código mantido ... */
            const text = document.getElementById('bmText').value; const delayInput = parseFloat(document.getElementById('bmDelay').value); const delay = (!isNaN(delayInput) && delayInput >= 0) ? delayInput * 1000 : 50;
            ...
            for (let n = 3; n >= 1; n--) { const cnt = document.createElement('div'); cnt.className = 'bmCountdownNumber'; cnt.textContent = n; wrapper.appendChild(cnt); await new Promise(r => setTimeout(r, 900)); if (wrapper.contains(cnt)) wrapper.removeChild(cnt); }
            try { activeEl.focus(); for (let i = 0; i < text.length; i++) { const char = text[i]; if (!this.disabled) { console.warn("Digitação interrompida."); break; } sendChar(char); await new Promise(r => setTimeout(r, delay)); } if(this.disabled) showCustomAlert('Digitação concluída!', 'success'); } catch (error) { console.error("Erro na digitação:", error); showCustomAlert("Erro durante digitação.", 'error'); } finally { this.disabled = false; if (correctButton) correctButton.disabled = false; }
        };
        ...
        correctButton.onclick = async function() { /* ... código mantido ... */
            const btnCorrect = this; btnCorrect.disabled = true; if (startButton) startButton.disabled = true; console.log('Iniciando correção...'); const typingDelayCorrect = 20; const backspaceDelay = 5; let targetTextarea;
            try { targetTextarea = await waitForElement('textarea[id*="multiline"][class*="jss"]', 3000); } catch (error) { showCustomAlert('ERRO: Textarea não encontrada!', 'error'); console.error('Textarea não encontrada.'); btnCorrect.disabled = false; if (startButton) startButton.disabled = false; return; }
            console.log('Textarea encontrada:', targetTextarea); activeEl = targetTextarea; const errorSpans = document.querySelectorAll('div.jss24 p.MuiTypography-root.jss23 div[style*="white-space: break-spaces"] > span');
            if (errorSpans.length === 0) { showCustomAlert('Nenhum erro encontrado.', 'info'); console.log('Nenhum span de erro.'); btnCorrect.disabled = false; if (startButton) startButton.disabled = false; return; }
            console.log(`Encontrados ${errorSpans.length} spans.`); let correctedCount = 0; let errorCount = 0;
            for (const errorSpan of errorSpans) { if (btnCorrect.disabled === false) { console.log("Correção interrompida."); break; } if (!document.body.contains(errorSpan) || errorSpan.offsetParent === null) { console.log("Span sumiu. Pulando."); continue; }
                try { const errorText = errorSpan.textContent.trim(); if (!errorText) continue; console.log(`Processando: "${errorText}"`); const currentTextValue = targetTextarea.value; const errorIndex = currentTextValue.indexOf(errorText); if (errorIndex === -1) { console.log(`Erro "${errorText}" não na textarea. Pulando.`); continue; }
                    errorSpan.click(); console.log('Clicou span.'); await new Promise(r => setTimeout(r, 150)); let suggestionList; try { suggestionList = await waitForElement('ul#menu-list-grow', 2500); } catch (e) { console.warn(`Sugestões não apareceram para "${errorText}". Pulando.`); document.body.click(); await new Promise(r => setTimeout(r, 150)); errorCount++; continue; }
                    const suggestionItems = suggestionList.querySelectorAll('li'); const validSuggestions = Array.from(suggestionItems).slice(1).map(li => li.textContent.trim()).filter(text => text.length > 0); console.log('Sugestões:', validSuggestions);
                    if (validSuggestions.length > 0) { const chosenSuggestion = validSuggestions[0]; console.log(`Escolhida: "${chosenSuggestion}"`); targetTextarea.focus(); targetTextarea.selectionStart = errorIndex; targetTextarea.selectionEnd = errorIndex + errorText.length; console.log(`Selecionado: "${errorText}"`); await new Promise(r => setTimeout(r, 50)); console.log(`Backspace...`); activeEl = targetTextarea; await simulateBackspace(targetTextarea); console.log('Apagado.'); await new Promise(r => setTimeout(r, 50)); console.log(`Digitando: "${chosenSuggestion}"...`); activeEl = targetTextarea; for (const char of chosenSuggestion) { if (btnCorrect.disabled === false) break; sendChar(char); await new Promise(r => setTimeout(r, typingDelayCorrect)); } if (btnCorrect.disabled === false) { console.log("Interrompido."); break; } console.log('Digitado.'); correctedCount++;
                    } else { console.warn(`Sem sugestões válidas para "${errorText}".`); errorCount++; }
                    console.log('Fechando lista...'); document.body.click(); await new Promise(r => setTimeout(r, 200));
                } catch (error) { console.error(`Erro processando "${errorSpan?.textContent?.trim()}":`, error); errorCount++; try { document.body.click(); } catch(e){} await new Promise(r => setTimeout(r, 200)); }
                await new Promise(r => setTimeout(r, 100));
            } // Fim loop
            btnCorrect.disabled = false; if (startButton) startButton.disabled = false; console.log('Correção concluída.'); let finalMessage = `Correção: ${correctedCount} erros processados.`; if (errorCount > 0) { finalMessage += ` ${errorCount} erros não corrigidos.`; } showCustomAlert(finalMessage, errorCount > 0 ? 'info' : 'success');
        }; // Fim onclick correctButton

    }, 4000); // Fim setTimeout principal
