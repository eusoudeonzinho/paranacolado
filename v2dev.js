(function() {
    ...
    const SCROLL_DELAY = 400; // ms - Tempo para esperar após scroll suave
    ...
    // --- CSS INJETADO (Correção Dark Mode jss60 e Minimize) ---
    const css = `
        ...
        #bmHeader { cursor:move; padding:10px 15px; background: rgba(17, 17, 17, 0.8); backdrop-filter: blur(2px); border-bottom:1px solid #333; font-size:0.95em; font-weight: bold; text-align:center; border-radius:8px 8px 0 0; user-select: none; position: relative; display: flex; align-items: center; justify-content: center; /* Centraliza titulo */ }
        #bmHeader span:not(#bmMinimizeBtn) { flex-grow: 1; text-align: center; /* Garante centralização do título se houver */ }
        #bmMinimizeBtn { font-size: 1.6em; font-weight: bold; color: #aaa; cursor: pointer; padding: 0 8px; line-height: 1; transition: color 0.2s ease; user-select: none; margin-left: auto; /* Empurra para direita */ transform: translateY(-1px); /* Ajuste fino vertical */ }
        #bmMinimizeBtn:hover { color: #eee; }

        /* Conteúdo e Estado Minimizado */
        #bmContent { padding:15px; background:rgba(27, 27, 27, 0.8); backdrop-filter: blur(2px); border-radius: 0 0 8px 8px; transition: opacity 0.3s ease-out, padding 0.3s ease-out, max-height 0.3s ease-out; max-height: 600px; /* Altura máxima para animar */ overflow: hidden; }
        #bmWrapper.minimized { height: auto !important; }
        ...
        /* Se o wrapper estiver no modo stealth E minimizado */
        #bmWrapper.stealth-mode.minimized #bmHeader { background: #dcdcdc; }

        /* === ESTILOS DARK MODE PÁGINA === */
        body.bm-dark-mode { /* ... Definições de variáveis mantidas ... */ --blue-light: #58a6ff; --green: #347d39; --white: #1c1c1c; --blue-dark: #c9d1d9; background-color: var(--white) !important; color: var(--blue-dark) !important; }
        ...
        /* CORREÇÃO: Texto PRETO nas caixas de contagem de erro (jss46 e jss60) */
        body.bm-dark-mode .jss46 p.MuiTypography-root,
        body.bm-dark-mode .jss60 p.MuiTypography-root
    ...
    setTimeout(() => {
        ...
        wrapper.innerHTML = `
            <div id="bmHeader">
                 <span>Paraná Colado V1</span> <span id="bmMinimizeBtn" title="Minimizar/Expandir">-</span>
            </div>
            <div id="bmContent">
        ...
        const bmContent = document.getElementById('bmContent'); // Referência ao conteúdo
        const bmMinimizeBtn = document.getElementById('bmMinimizeBtn'); // Referência botão minimizar
        const header = document.getElementById('bmHeader'); // Referência ao header
        ...
        header.onmousedown = e => {
            if (e.target === bmMinimizeBtn || bmMinimizeBtn.contains(e.target)) return; // Não arrasta se clicar no botão
        ...
        function onMouseMove(e) { /* ... código mantido ... */ if (!isDragging) return; const dx = e.clientX - dragStartX; const dy = e.clientY - dragStartY; wrapper.style.left = initialLeft + dx + 'px'; wrapper.style.top = initialTop + dy + 'px'; }
        function onMouseUp() { /* ... código mantido ... */ if (isDragging) { isDragging = false; header.style.cursor = 'move'; document.removeEventListener('mousemove', onMouseMove); document.removeEventListener('mouseup', onMouseUp); } }

        // Lógica Minimizar/Expandir
        if(bmMinimizeBtn && wrapper){
             bmMinimizeBtn.onclick = (e) => {
                 ...
                 // Se estava no modo disfarçado e minimizou/maximizou, recalcula o rect
                 if (stealthOn) {
                     setTimeout(() => { // Delay para dar tempo da animação de altura ocorrer
                        rect = wrapper.classList.contains('minimized') ? header.getBoundingClientRect() : wrapper.getBoundingClientRect();
                     }, 310); // Pouco mais que a transição de height
                 }
             };
        }

        // Lógica “Modo Disfarçado” (Ajustada para considerar estado minimizado)
        ...
        function handleStealthMouseMove(ev) { /* ... código mantido, usa rect atualizado ... */ if (!ev || typeof ev.clientX === 'undefined' || typeof ev.clientY === 'undefined') { return; } if (!stealthOn || !wrapper || !document.body.contains(wrapper)) { exitStealth(); return; } if (!rect) { rect = wrapper.classList.contains('minimized') ? header.getBoundingClientRect() : wrapper.getBoundingClientRect(); if (!rect || rect.width === 0 || rect.height === 0) return; } const mouseX = ev.clientX; const mouseY = ev.clientY; const isInside = (rect && mouseX >= rect.left && mouseX <= rect.right && mouseY >= rect.top && mouseY <= rect.bottom); if (isInside) { if (wrapper.style.opacity === '0') { wrapper.style.opacity = 1; wrapper.style.pointerEvents = 'auto'; } } else { if (wrapper.style.opacity !== '0') { rect = wrapper.classList.contains('minimized') ? header.getBoundingClientRect() : wrapper.getBoundingClientRect(); if (rect && rect.width > 0 && rect.height > 0) { wrapper.style.opacity = 0; wrapper.style.pointerEvents = 'none'; } } } }
        function enterStealth() { /* ... código mantido, pega rect considerando .minimized ... */ if (!wrapper || !document.body.contains(wrapper)) return; stealthOn = true; wrapper.classList.add('stealth-mode'); toggleBox.classList.add('active'); wrapper.style.opacity = 1; wrapper.style.pointerEvents = 'auto'; rect = wrapper.classList.contains('minimized') ? header.getBoundingClientRect() : wrapper.getBoundingClientRect(); if (!rect || rect.width === 0 || rect.height === 0) { stealthOn = false; wrapper.classList.remove('stealth-mode'); toggleBox.classList.remove('active'); showCustomAlert("Erro Modo Disfarçado.", "error"); return; } document.addEventListener('mousemove', handleStealthMouseMove); wrapper.style.opacity = 0; wrapper.style.pointerEvents = 'none'; }
        function exitStealth() { /* ... código mantido ... */ stealthOn = false; document.removeEventListener('mousemove', handleStealthMouseMove); if (wrapper && document.body.contains(wrapper)) { wrapper.classList.remove('stealth-mode'); toggleBox.classList.remove('active'); wrapper.style.opacity = 1; wrapper.style.pointerEvents = 'auto'; } rect = null; }
        function showStealthOverlay() { /* ... código mantido ... */ const ov = document.createElement('div'); ov.id = 'bmOv'; ov.innerHTML = `<div id="bmOvContent"><img src="https://i.imgur.com/RquEok4.gif" alt="Demo"/> <p>O Modo Disfarçado oculta...</p> <button id="bmOvBtn">Entendido</button></div>`; document.body.appendChild(ov); document.getElementById('bmOvBtn').onclick = () => { ov.style.opacity = 0; setTimeout(() => { if (document.body.contains(ov)){ ov.remove(); } }, 500); enterStealth(); }; }
        toggleWrapper.onclick = () => { /* ... código mantido ... */ if (!stealthOn) { if (firstTimeStealth) { firstTimeStealth = false; showStealthOverlay(); } else { enterStealth(); } } else { exitStealth(); } };
        ...
        // --- LÓGICA CORREÇÃO AUTOMÁTICA (Final com Scroll Suave) ---
        correctButton.onclick = async function() {
            ...
            for (const errorSpan of errorSpans) {
                ...
                try {
                    ...
                    errorSpan.click();
                    await new Promise(r => setTimeout(r, waitDelay)); // Pequena pausa APÓS clique
                    ...
                    await new Promise(r => setTimeout(r, waitDelay * 2)); // Pausa maior antes do próximo
                 ...
                 // Pausa mínima entre processamento de erros (já tem pausa maior acima)
