(function() {
    ...
    // --- Constantes ---
    const MIN_DELAY = 1;
    const SCROLL_DELAY = 50; // Mantido rápido para funcionalidade
    const STEP_DELAY = MIN_DELAY; // Mantido rápido
    const MIN_WRAPPER_WIDTH = 150;
    const MIN_WRAPPER_HEIGHT = 130; // Mantido para resize
    ...
    function showCustomAlert(message, type = 'info') {
        ...
        alertBox.classList.add(`bmAlert-${type}`); // Adiciona classe de tipo

        // Adiciona Ícone baseado no tipo
        let iconHtml = '';
        switch (type) {
            case 'error': iconHtml = '<div class="bmAlertIcon error">!</div>'; break;
            case 'warning': iconHtml = '<div class="bmAlertIcon warning">!</div>'; break;
            case 'success': iconHtml = '<div class="bmAlertIcon success">✓</div>'; break;
            case 'info': // no break
            default: iconHtml = '<div class="bmAlertIcon info">i</div>'; break;
        }
        ...
        closeBtn.onclick = () => {
            alertBox.classList.remove('bmAlertEnter'); // Prepara para animação de saída
            alertBox.classList.add('bmAlertExit');
            overlay.classList.add('bmAlertFadeOut');
            setTimeout(() => {
                if (document.body.contains(overlay)) { document.body.removeChild(overlay); }
            }, 400); // Duração da animação de saída
        };
        alertBox.innerHTML = iconHtml; // Adiciona o ícone
        alertBox.appendChild(messageP);
        alertBox.appendChild(closeBtn);
        overlay.appendChild(alertBox);
        document.body.appendChild(overlay);
        void alertBox.offsetWidth; // Reflow
        overlay.classList.add('bmAlertFadeIn');
        alertBox.classList.add('bmAlertEnter');
    }
    ...
    // --- SPLASH INICIAL (MEGA ANIMADO) ---
    const splash = document.createElement('div'); splash.id = 'bmSplash';
    // Conteúdo simplificado, foco na animação CSS
    splash.innerHTML = `
        ...
        <div class="bmSplashGrid"></div> `;
    document.body.appendChild(splash);

    // --- CSS INJETADO (ANIMAÇÃO TOTAL!) ---
    const css = `
        /* --- Splash Screen MEGA Animado --- */
        #bmSplashBgEffect { position: absolute; inset: 0; overflow: hidden; z-index: 1; background: radial-gradient(circle, #2a1a4f 0%, #0a0514 70%); opacity: 0; animation: bgSplashEnter 5s ease-out forwards; }
        @keyframes bgSplashEnter { 0% { opacity: 0; transform: scale(1.1); } 50% { opacity: 1; transform: scale(1); } 100% { opacity: 1; } }
        /* Efeito Grid Sutil */
        .bmSplashGrid { position: absolute; inset: -200px; z-index: 2; background-image: linear-gradient(rgba(138, 43, 226, 0.1) 1px, transparent 1px), linear-gradient(90deg, rgba(138, 43, 226, 0.1) 1px, transparent 1px); background-size: 50px 50px; opacity: 0; animation: gridFadeMove 5s ease-out forwards 0.2s; }
        @keyframes gridFadeMove { 0% { opacity: 0; background-position: 0 0; } 50% { opacity: 0.5; } 100% { opacity: 0.3; background-position: -100px -100px; } }

        #bmSplash { position: fixed; inset: 0; background:transparent; /* Fundo vem do BgEffect */ display:flex; align-items:center; justify-content:center; z-index:99999; overflow:hidden; animation: splashHide 0.8s cubic-bezier(0.6, -0.28, 0.735, 0.045) forwards 5.2s; /* Duração total maior */ }
        #bmSplashContent { z-index: 3; display:flex; flex-direction:column; align-items:center; justify-content:center; perspective: 1000px; }
        #bmSplashImg { width:190px; margin-bottom: 30px; filter: drop-shadow(0 8px 30px rgba(160, 86, 247, 0.7)); opacity: 0; transform: scale(0.6) rotateZ(-30deg) translateY(50px); animation: logoSuperEntry 2.2s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards 0.5s, logoFloatBob 2s ease-in-out infinite alternate 2.7s; }
        @keyframes logoSuperEntry {
            0% { opacity: 0.5; transform: scale(0.6) rotateZ(-30deg) translateY(50px); }
            60% { opacity: 1; transform: scale(1.15) rotateZ(10deg) translateY(0px); }
            80% { transform: scale(0.95) rotateZ(-5deg); }
            100% { opacity: 1; transform: scale(1) rotateZ(0deg); }
        }
        @keyframes logoFloatBob { from { transform: translateY(0px) scale(1); filter: drop-shadow(0 8px 30px rgba(160, 86, 247, 0.7)); } to { transform: translateY(-8px) scale(1.02); filter: drop-shadow(0 12px 35px rgba(160, 86, 247, 0.8)); } }

        #bmSplashTitle, #bmSplashSubtitle { font-family:'Segoe UI Black', Arial, sans-serif; color:#fff; text-shadow: 0 0 12px rgba(220, 180, 255, 0.8); opacity: 0; transform: translateY(25px) scale(0.95); animation: textSuperAppear 1s cubic-bezier(0.23, 1, 0.32, 1) forwards; }
        #bmSplashTitle { font-size: 3em; font-weight: 900; letter-spacing: 1px; margin-bottom: 6px; animation-delay: 2.5s; background: linear-gradient(45deg, #d8baff, #a056f7); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
        #bmSplashSubtitle { font-size: 1.5em; font-weight: 300; color: #e0cffc; font-family:'Segoe UI Light', Arial, sans-serif; letter-spacing: 0.8px; animation-delay: 2.7s; }
        @keyframes textSuperAppear { to { opacity: 1; transform: translateY(0) scale(1); } }

        #bmLoadingBar { width: 280px; height: 8px; background-color: rgba(255, 255, 255, 0.1); border-radius: 4px; margin-top: 45px; overflow: hidden; opacity: 0; transform: scaleY(0.5); animation: barSuperAppear 0.7s ease-out forwards 3.5s; box-shadow: inset 0 1px 2px rgba(0,0,0,0.3); }
        #bmLoadingProgress { width: 0%; height: 100%; background: linear-gradient(90deg, #a056f7, #f8c3ff); border-radius: 4px; animation: loadingAnimFinal 1.3s cubic-bezier(0.65, 0.05, 0.36, 1) forwards 3.6s; /* Começa junto com fadeout */ position: relative; overflow: hidden;}
        /* Efeito shimmer na barra */
         #bmLoadingProgress::after { content: ''; position: absolute; top: 0; left: -50%; width: 50%; height: 100%; background: linear-gradient(to right, rgba(255,255,255,0) 0%, rgba(255,255,255,0.4) 50%, rgba(255,255,255,0) 100%); transform: skewX(-25deg); animation: shimmer 1.5s infinite; animation-delay: 3.8s;}
        @keyframes barSuperAppear { to { opacity: 1; transform: scaleY(1); } }
        @keyframes loadingAnimFinal { from { width: 0%; } to { width: 100%; } }
        @keyframes shimmer { 0% { left: -70%; } 100% { left: 120%; } }
        @keyframes splashHide { from { opacity: 1; transform: scale(1); } to { opacity: 0; transform: scale(0.9); visibility: hidden; } }

        /* Alertas (Com entrada mais dinâmica) */
        #bmAlertOverlay { position: fixed; inset: 0; background: rgba(0, 0, 0, 0.75); backdrop-filter: blur(5px); -webkit-backdrop-filter: blur(5px); display: flex; align-items: center; justify-content: center; z-index: 100001; opacity: 0; pointer-events: none; transition: opacity 0.4s ease-out; }
        #bmAlertOverlay.bmAlertFadeIn { opacity: 1; pointer-events: auto; }
        #bmAlertOverlay.bmAlertFadeOut { opacity: 0; pointer-events: none; }
        #bmAlertBox { background: linear-gradient(145deg, #2a2a2e, #1f1f21); color: #fff; padding: 25px 35px 30px 35px; border-radius: 10px; border: 1px solid #444; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.6); min-width: 320px; max-width: 480px; text-align: center; font-family: 'Segoe UI', sans-serif; opacity: 0; transform: scale(0.8) translateY(-20px); transition: opacity 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275), transform 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); }
        #bmAlertBox.bmAlertEnter { opacity: 1; transform: scale(1) translateY(0); }
        #bmAlertBox.bmAlertExit { opacity: 0; transform: scale(0.9) translateY(10px); }
        /* Ícones do Alerta */
        .bmAlertIcon { width: 40px; height: 40px; border-radius: 50%; margin: 0 auto 15px auto; display: flex; align-items: center; justify-content: center; font-size: 1.5em; font-weight: bold; color: #fff; box-shadow: 0 3px 8px rgba(0,0,0,0.3); }
        .bmAlertIcon.info { background-color: #58a6ff; }
        .bmAlertIcon.success { background-color: #56d364; }
        .bmAlertIcon.warning { background-color: #f1c40f; color: #333; }
        .bmAlertIcon.error { background-color: #e74c3c; }
        #bmAlertMessage { font-size: 1.15em; line-height: 1.6; margin: 0 0 25px 0; color: #eee; }
        #bmAlertCloseBtn { /* Estilo similar aos botões principais */ padding: 12px 30px; font-size: 1em; font-weight: bold; background: linear-gradient(145deg, #9a3bf6, #7022b6); border: none; border-radius:8px; color: #fff; text-shadow: 0 1px 3px rgba(0,0,0,0.4); cursor:pointer; transition: all 0.15s ease-out; box-shadow: 0 4px 9px rgba(0,0,0,0.6), inset 0 1px 1px rgba(255,255,255,0.15); letter-spacing: 0.5px; }
        #bmAlertCloseBtn:hover { filter: brightness(1.2) saturate(1.2); transform: translateY(-2px) scale(1.03); box-shadow: 0 7px 14px rgba(138, 43, 226, 0.5), inset 0 1px 1px rgba(255,255,255,0.25); }
        #bmAlertCloseBtn:active { transform: translateY(0px) scale(0.98); filter: brightness(0.9); box-shadow: 0 2px 5px rgba(138, 43, 226, 0.4), inset 0 1px 2px rgba(0,0,0,0.2); }

        /* UI Principal (Super Linda) */
        #bmWrapper {
            box-sizing: border-box; position:fixed; top:20px; right:20px; width:310px; /* Levemente mais fino */ border:1px solid #505055; border-radius:14px; /* Mais redondo */ box-shadow:0 12px 40px rgba(0,0,0,0.8); font-family:'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; color:#f0f0f0; opacity:0; transform:translateX(80px) rotateY(-20deg) scale(0.9); transition:opacity 0.7s cubic-bezier(0.165, 0.84, 0.44, 1), transform 0.7s cubic-bezier(0.165, 0.84, 0.44, 1); z-index:99998; overflow: hidden; /* Revertido para hidden, resize removido */ background: linear-gradient(150deg, #2c2c31, #1e1e20); /* Gradiente mais escuro */
        }
        #bmWrapper.show { opacity:1; transform:translateX(0) rotateY(0deg) scale(1); }
        #bmWrapper > div:not(#bmResizeHandle) { border-radius: inherit; overflow: hidden; }
        #bmHeader { cursor:move; padding:14px 20px; background: rgba(10, 10, 12, 0.9); backdrop-filter: blur(6px); border-bottom:1px solid #505055; font-size:1.1em; font-weight: 600; text-align:center; border-radius:14px 14px 0 0; user-select: none; position: relative; display: flex; align-items: center; justify-content: center; } #bmHeader span:not(#bmMinimizeBtn) { flex-grow: 1; text-align: center; color: #f5f5f5; text-shadow: 0 1px 1px rgba(0,0,0,0.6); } #bmMinimizeBtn { font-size: 1.7em; font-weight: bold; color: #ccc; cursor: pointer; padding: 0 8px; line-height: 1; transition: color 0.2s ease, transform 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275); user-select: none; margin-left: auto; transform: translateY(-1px) rotate(0deg); } #bmMinimizeBtn:hover { color: #fff; transform: translateY(-1px) scale(1.15) rotate(180deg); } #bmWrapper.minimized #bmMinimizeBtn { transform: translateY(-1px) rotate(180deg); } #bmWrapper.minimized #bmMinimizeBtn:hover { transform: translateY(-1px) scale(1.15) rotate(0deg); }
        #bmContent { padding: 22px; background:rgba(42, 42, 46, 0.95); backdrop-filter: blur(6px); border-radius: 0 0 14px 14px; transition: opacity 0.3s ease-out, padding 0.3s ease-out, max-height 0.4s cubic-bezier(0.4, 0, 0.2, 1); max-height: 700px; overflow-y: auto; overflow-x: hidden; }
        #bmWrapper.minimized { height: auto !important; min-height: 0 !important; background: rgba(15, 15, 17, 0.98); border-color: #555; } #bmWrapper.minimized #bmContent { opacity: 0; padding-top: 0; padding-bottom: 0; max-height: 0; border-width: 0; margin: 0; overflow: hidden; } #bmWrapper.minimized #bmHeader { border-bottom: none; border-radius: 14px; }
        /* Handle de Resize Removido */
        #bmContent textarea, #bmContent input[type="number"] { width:100%; margin-bottom:18px; padding:15px; font-size:1.05em; font-family: Consolas, Monaco, 'Andale Mono', 'Ubuntu Mono', monospace; background:rgba(18, 18, 20, 0.9); border:1px solid #5f5f66; border-radius:10px; color:#f8f8f8; box-sizing:border-box; resize: vertical; transition: border-color 0.3s ease, box-shadow 0.3s ease, background-color 0.3s ease; box-shadow: inset 0 2px 6px rgba(0,0,0,0.7); } #bmContent textarea { min-height: 110px; } #bmContent textarea:focus, #bmContent input[type="number"]:focus { outline:none; border-color: #c89bff; background-color: rgba(0,0,0,0.7); box-shadow: 0 0 0 4px rgba(199, 155, 255, 0.25), inset 0 2px 6px rgba(0,0,0,0.7); }
        #bmContent button { width:100%; padding:15px; margin-top: 15px; font-size:1.1em; font-weight: bold; background: linear-gradient(145deg, #a056f7, #7a2fd0); border: none; border-radius:10px; color: #fff; text-shadow: 0 1px 3px rgba(0,0,0,0.5); cursor:pointer; transition: all 0.15s ease-out; box-sizing: border-box; box-shadow: 0 5px 12px rgba(0,0,0,0.6), inset 0 1px 1px rgba(255,255,255,0.2); letter-spacing: 0.6px; } #bmContent button:disabled { cursor: not-allowed; opacity: 0.4; background: #555 !important; box-shadow: inset 0 2px 4px rgba(0,0,0,0.4) !important; transform: none !important; color: #999; filter: grayscale(80%); } #bmContent button:not(:disabled):hover { filter: brightness(1.25) saturate(1.2); transform: translateY(-4px) scale(1.02); box-shadow: 0 9px 18px rgba(138, 43, 226, 0.6), inset 0 1px 1px rgba(255,255,255,0.3); } #bmContent button:not(:disabled):active { transform: translateY(-1px) scale(0.98); filter: brightness(0.9); box-shadow: 0 3px 6px rgba(138, 43, 226, 0.5), inset 0 1px 3px rgba(0,0,0,0.3); }
        #bmToggleWrapper, #bmDarkModeToggleWrapper { display:flex; align-items:center; gap:14px; margin-bottom:18px; cursor: pointer; padding: 10px 12px; border-radius: 10px; transition: background-color 0.25s ease; } #bmToggleWrapper:hover, #bmDarkModeToggleWrapper:hover { background-color: rgba(138, 43, 226, 0.2); } #bmToggleImg, #bmDarkModeToggleImg { width:22px; height:22px; border:2px solid #a056f7; border-radius:6px; background:transparent; transition: all .3s cubic-bezier(0.175, 0.885, 0.32, 1.275); display: flex; align-items: center; justify-content: center; flex-shrink: 0; position: relative; } #bmToggleImg.active::after, #bmDarkModeToggleImg.active::after { content: '✔'; position: absolute; font-size: 16px; color: #fff; text-shadow: 0 0 3px #000; opacity: 0; transform: scale(0.5) rotate(-180deg); animation: checkSuperAppear 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards 0.1s; } #bmToggleImg.active, #bmDarkModeToggleImg.active { background: #a056f7; border-color: #c89bff; transform: rotate(15deg) scale(1.05); } @keyframes checkSuperAppear { to { opacity: 1; transform: scale(1) rotate(0deg); } } #bmToggleText, #bmDarkModeToggleText { font-size:1.1em; color:#f0f0f0; user-select:none; line-height: 1.2; font-weight: 500; }
        /* Countdowns */ .bmCountdownNumber { /* ... */ position: absolute; bottom: 80px; left: 50%; transform: translateX(-50%); font-family: 'Segoe UI Black', sans-serif; color: #8A2BE2; font-size: 3em; opacity: 0; animation: countPopZoom 0.8s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards; z-index: 10; text-shadow: 0 0 10px rgba(138, 43, 226, 0.7); } @keyframes countPopZoom { 0% { opacity: 0; transform: translateX(-50%) scale(0.5) rotate(-15deg); } 60% { opacity: 1; transform: translateX(-50%) scale(1.1) rotate(5deg); } 100% { opacity: 0; transform: translateX(-50%) scale(1) rotate(0deg); } } .bmCorrectionCountdownNumber { /* ... */ position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); font-family: 'Segoe UI Black', sans-serif; color: #4ecdc4; font-size: 5em; opacity: 0; animation: correctionCountPop 0.9s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards; z-index: 100002; text-shadow: 0 0 15px rgba(78, 205, 196, 0.7); pointer-events: none; } @keyframes correctionCountPop { 0% { opacity: 0; transform: translate(-50%, -50%) scale(0.6) rotate(10deg); } 60% { opacity: 1; transform: translate(-50%, -50%) scale(1.1) rotate(-3deg); } 100% { opacity: 0; transform: translate(-50%, -50%) scale(1) rotate(0deg); } }
        ...
        /* Splash Correção */
        #bmCorrectionSplash { position: fixed; inset: 0; background: #0A0514; backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); display: flex; flex-direction: column; align-items: center; justify-content: center; z-index: 100000; opacity: 0; transition: opacity 0.5s ease-out; font-family: 'Segoe UI', sans-serif; color: #eee; } #bmCorrectionSplash.visible { opacity: 1; } #bmCorrectionSplash .splash-content { text-align: center; transform: scale(0.9); opacity: 0; animation: splashContentEntry 0.6s cubic-bezier(0.175, 0.885, 0.32, 1.275) 0.1s forwards; } #bmCorrectionSplash h2 { font-size: 1.9em; font-weight: 600; color: #d5c2f7; margin-bottom: 30px; text-shadow: 0 0 10px rgba(160, 86, 247, 0.6); } #bmCorrectionSplash .word-display { display: flex; align-items: center; justify-content: center; gap: 25px; margin-bottom: 35px; min-height: 90px; perspective: 500px; } #bmCorrectionSplash .word-box { background: rgba(255, 255, 255, 0.07); border: 1px solid rgba(255, 255, 255, 0.15); border-radius: 10px; padding: 18px 30px; min-width: 160px; text-align: center; box-shadow: 0 4px 15px rgba(0,0,0,0.3); } #bmCorrectionSplash .word-box .label { font-size: 0.85em; color: #aaa; margin-bottom: 10px; text-transform: uppercase; letter-spacing: 0.8px; } #bmCorrectionSplash .word-box .word { font-size: 1.7em; font-weight: 500; color: #fff; min-height: 1.5em; display: inline-block; } #bmCorrectionSplash .word-arrow { font-size: 2.8em; color: #8A2BE2; opacity: 0.9; transform: scale(1); transition: transform 0.3s ease; } #bmCorrectionSplash .word-display:hover .word-arrow { transform: scale(1.1); }
        #bmCorrectionSplash .word { opacity: 1; transform: rotateX(0deg); transition: opacity 0.25s ease-out, transform 0.35s cubic-bezier(0.175, 0.885, 0.32, 1.275); } #bmCorrectionSplash .word.exiting { opacity: 0; transform: rotateX(70deg) translateY(15px); } #bmCorrectionSplash .word.entering { opacity: 0; transform: rotateX(-70deg) translateY(-15px); animation: wordEnterRotate 0.35s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards 0.05s; } @keyframes wordEnterRotate { to { opacity: 1; transform: rotateX(0deg) translateY(0); } }
        #bmCorrectionSplash .spinner { width: 45px; height: 45px; border: 5px solid rgba(255, 255, 255, 0.2); border-left-color: #a056f7; border-radius: 50%; animation: spin 0.8s linear infinite; margin: 15px auto 25px auto; } @keyframes spin { to { transform: rotate(360deg); } } #bmCorrectionSplash .applying-text { font-size: 1.4em; color: #ddd; letter-spacing: 0.5px; } @keyframes splashContentEntry { from { transform: scale(0.9) translateY(15px); opacity: 0; } to { transform: scale(1) translateY(0); opacity: 1; } }
    ...
    setTimeout(() => {
        ...
        correctButton.onclick = async function() {
            ...
            // const waitDelay = STEP_DELAY; // REMOVIDO, usar MIN_DELAY
            ...
            // --- Loop Principal de Correção (SUPER RÁPIDO + Scroll Reforçado) ---
            for (const errorSpan of errorSpans) {
                ...
                try {
                    ...
                    if (SCROLL_DELAY > 0) await new Promise(r => setTimeout(r, SCROLL_DELAY)); // Pausa MÍNIMA scroll
                    ...
                    errorSpan.click(); await new Promise(r => setTimeout(r, MIN_DELAY)); // Pausa mínima

                    let suggestionList;
                    try { suggestionList = await waitForElement('ul#menu-list-grow', 500); }
                    catch (e) { errorCount++; await updateCorrectionSplash(errorText, "Sem Sugestão"); document.body.click(); await new Promise(r => setTimeout(r, MIN_DELAY)); continue; }
                    ...
                    if (validSuggestions.length > 0) {
                        ...
                        await updateCorrectionSplash(errorText, chosenSuggestion);
