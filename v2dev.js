(function() {
    ...
    // Nova função para esperar um elemento desaparecer
        ...
        return new Promise((resolve, reject) => {
            ...
            const intervalId = setInterval(() => {
                ...
                if (!element) {
                    ...
                    clearTimeout(timeoutId); // Limpa o timeout geral
                    resolve("Elemento desapareceu");
                }
            ...
            const timeoutId = setTimeout(() => {
                clearInterval(intervalId);
                console.error(`Timeout ${timeout}ms esperando ${selector} desaparecer.`);
                reject(new Error(`Timeout esperando ${selector} desaparecer`));
            }, timeout);
        });
    }
    ...
    // --- CSS INJETADO (Removido estilo #bmTypingIndicator) ---
    const css = ` /* ... CSS Completo Mantido, EXCETO #bmTypingIndicator e seus filhos ... */
        ...
        /* Estilos para #bmTypingIndicator foram REMOVIDOS */
    `;
    ...
    const splashTimeout = 3800; // Pode reduzir um pouco se a animação terminar antes
    setTimeout(() => {
        ...
        wrapper.innerHTML = `
            ...
            <div id="bmContent">
                ...
                <input id="bmDelay" type="number" step="0.001" value="0.001" min="0.001" placeholder="Delay (s)"> <div id="bmToggleWrapper"> <div id="bmToggleImg"></div> <span id="bmToggleText">Modo Disfarçado</span> </div>
        ...
        // Lógica Botão "Iniciar Digitação" (REMOVIDO Indicador "Escrevendo", Acelerado)
        const startButton = document.getElementById('bmBtn');
        const correctButton = document.getElementById('bmBtnCorrect');
        // Não precisamos mais de shouldStopTyping ou typingIndicatorElement
        ...
        startButton.onclick = async function() {
            ...
            // --- Contagem Regressiva (Mantida) ---
            for (let n = 3; n >= 1; n--) {
                const cnt = document.createElement('div'); cnt.className = 'bmCountdownNumber'; cnt.textContent = n;
                wrapper.appendChild(cnt);
                // A pausa da contagem é visual, não precisa ser MIN_DELAY
                await new Promise(r => setTimeout(r, 700)); // Reduzido um pouco
                if (wrapper.contains(cnt)) wrapper.removeChild(cnt);
                await new Promise(r => setTimeout(r, 100)); // Pausa entre números
            }

            // --- Loop de Digitação (Sem indicador, muito rápido) ---
            let typingCompleted = true; // Assumir sucesso inicialmente
            try {
                activeEl.focus(); // Garante foco no elemento alvo
                for (let i = 0; i < text.length; i++) {
                    const char = text[i];
                    const success = sendChar(char);
                    if (!success) {
                        console.error("Erro ao enviar caractere, interrompendo digitação.");
                        typingCompleted = false; // Marcar que falhou
                        break; // Interrompe o loop
                    }
                    // Pausa mínima entre caracteres
                ...
                if (typingCompleted) {
                    showCustomAlert('Digitação concluída!', 'success');
                     ...
                     showCustomAlert('Digitação interrompida por erro.', 'error');
                }
        ...
        // --- LÓGICA CORREÇÃO AUTOMÁTICA (MODIFICADA para velocidade e esperar 'PROCESSANDO' sumir) ---
        ...
        //waitForChangeOrTimeout REMOVIDA
        ...
        correctButton.onclick = async function() {
            ...
            const backspaceDelay = MIN_DELAY; // simulateBackspace já usa MIN_DELAY
            ...
            try {
                ...
                for (const button of buttons) {
                    // Verifica o texto EXATO (removendo espaços extras)
                ...
                if (foundButton) {
                    ...
                    correctorButtonClicked = true;

                    // Espera o elemento 'PROCESSANDO' desaparecer
                    const processingSelector = 'div.sc-kAyceB.kEYIQb'; // Seletor do elemento de processamento
                    ...
                    console.log("Aguardou 3 segundos. Iniciando correção dos spans...");

                } else {
                    console.log("Botão 'CORRIGIR ONLINE' não encontrado. Prosseguindo diretamente.");
                }
            } catch (error) {
                if (error.message.includes('Timeout')) {
                    showCustomAlert("Timeout esperando 'PROCESSANDO' desaparecer. Verifique a página e tente novamente.", 'error');
                } else {
                ...
                // Parar ou Continuar? Decide-se parar aqui para evitar correções inesperadas.
            ...
            for (const errorSpan of errorSpans) {
                ...
                try {
                    ...
                    let suggestionList;
                    try { suggestionList = await waitForElement('ul#menu-list-grow', 1000); } // Timeout bem curto
                    ...
                    if (validSuggestions.length > 0) {
                        ...
                        activeEl = targetTextarea;
                        for (const char of chosenSuggestion) { if (btnCorrect.disabled === false) break; sendChar(char); if(typingDelayCorrect > 0) await new Promise(r => setTimeout(r, typingDelayCorrect)); }
