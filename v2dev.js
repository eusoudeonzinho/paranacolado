...
(function () {
    ...
    function dispatchKeyEvent(target, eventType, key, keyCode, charCode = 0, ctrlKey = false, altKey = false, shiftKey = false, metaKey = false) {
        ...
        const event = new KeyboardEvent(eventType, {
            key: key,
            code: `Key${key.toUpperCase()}`, // Nota: Pode não ser preciso para teclas como Backspace/Control
            keyCode: keyCode,
            which: keyCode,
            ...
            altKey: altKey,
            shiftKey: shiftKey,
            metaKey: metaKey // Para Cmd no Mac
        });
        try {
            target.dispatchEvent(event);
            // console.log(`Evento despachado: ${eventType}, Key: ${key}, Code: ${event.code}, Ctrl: ${ctrlKey}`);
        } catch (e) {
    ...
    async function simulateBackspace(targetElement) {
        ...
        activeEl = targetElement; // Garante que activeEl é o correto
        // Não precisa focar explicitamente aqui se já focou antes (ex: select())

        dispatchKeyEvent(targetElement, 'keydown', 'Backspace', 8);

        let valueChanged = false;
        // A lógica de manipulação direta do valor é mais confiável para backspace pós-seleção
        if (targetElement.tagName === 'INPUT' || targetElement.tagName === 'TEXTAREA') {
            ...
            // Se algo está selecionado (start !== end), o backspace deve apagar a seleção
            if (start !== end) {
                newValue = currentValue.substring(0, start) + currentValue.substring(end);
                newCursorPos = start; // Cursor fica no início da antiga seleção
                valueChanged = true;
            }
            // Se nada selecionado, mas cursor não no início, apaga caractere anterior
            else if (start > 0) {
            ...
            if (valueChanged) {
                 try {
                     ...
                     console.warn("Erro ao definir valor/disparar evento no backspace simulado", e);
             ...
             // Para contentEditable, execCommand delete pode funcionar após seleção
             document.execCommand('delete', false, null);
             valueChanged = true; // Assume que mudou
        }
    ...
    async function clearTextareaSimulated(textareaElement) {
        ...
        console.log("Iniciando limpeza rápida da textarea (Select All + Backspace)...");
        activeEl = textareaElement; // Garante que activeEl é o correto
        try {
            textareaElement.focus({ preventScroll: true });
            await new Promise(r => setTimeout(r, 50)); // Pequena pausa após focar

            // Seleciona todo o texto
            textareaElement.select();
            console.log("Texto selecionado.");
            await new Promise(r => setTimeout(r, 50)); // Pequena pausa após selecionar

            // Simula UM Backspace (que deve apagar a seleção)
            await simulateBackspace(textareaElement);
            console.log("Backspace simulado.");

            // Verificação final
            if (textareaElement.value !== "") {
                console.warn("Limpeza com select/backspace falhou, forçando valor vazio.");
                const prototype = Object.getPrototypeOf(textareaElement);
                const descriptor = Object.getOwnPropertyDescriptor(prototype, 'value');
                 try {
                     if (descriptor && descriptor.set) { descriptor.set.call(textareaElement, ""); }
                     ...
                     textareaElement.value = ""; // Fallback final
                 }
            } else {
                console.log("Textarea limpa com sucesso.");
            }
        } catch (error) {
            console.error("Erro durante clearTextareaSimulated:", error);
            // Tenta forçar limpeza em caso de erro
             try {
    ...
    function removeOverlay(elementOrId) {
        ...
        if (overlayElement && document.body.contains(overlayElement)) {
            ...
            const contentBox = overlayElement.querySelector('.bmAdvSplashContent'); // Classe comum
             if(contentBox) {
    ...
    function updateAIProgressBar(progressBarElement, targetPercentage) {
        if (progressBarElement) {
            ...
            progressBarElement.style.width = `${clampedPercentage}%`;
            // console.log(`Progresso IA atualizado para: ${clampedPercentage}%`);
        }
    }
    ...
    // --- CSS INJETADO ---
    const css = `
        /* ... (TODO O CSS ANTERIOR É MANTIDO EXATAMENTE IGUAL - V1.11) ... */
    ...
    setTimeout(() => {
        ...
        // --- LÓGICA CORREÇÃO AUTOMÁTICA (ATUALIZADA) ---
        ...
        // Modificado para usar removeOverlay e limpar a referência global
        function hideAdvancedCorrectionSplash() {
             if (correctionSplashEl) {
                 removeOverlay(correctionSplashEl); // Usa a função genérica
                 correctionSplashEl = null; // Limpa a referência global
             }
         }

        // --- Evento Click do Botão Corrigir (ATUALIZADO) ---
        correctButton.onclick = async function () {
            ...
            // Limpa overlays anteriores antes de começar
            removeOverlay('bmAIReviewSplash');
            removeOverlay('bmAILoadingSplash');
            hideAdvancedCorrectionSplash(); // Garante que o splash de correção saia

            try {
                correctionMode = await showModeSelectionDialog();
                 removeOverlay('bmModeSelectionDialog'); // Remove o diálogo de modo
                if (correctionMode === 'basic') {
                    ...
                    const confirmBasic = await showBasicModeConfirmationDialog();
                     removeOverlay('bmBasicConfirmDialog'); // Remove o diálogo de confirmação
                    ...
                    console.log('Modo Básico confirmado.');
                     ...
                     // Mostra o splash SÓ se for modo avançado
                     showAdvancedCorrectionSplash(".");
                     await new Promise(r => setTimeout(r, 300)); // Pequena pausa para exibir
                } else {
                ...
                try {
                    // --- REMOÇÃO DO COUNTDOWN ---
                    // O bloco 'if (needsCountdown)' foi removido daqui.
                    // A variável needsCountdown ainda pode ser útil para outras lógicas se necessário,
                    // mas o loop do countdown visual não será executado.
                    let needsCorrectorClick = false; // Renomeado de needsCountdown
                    // Lógica de verificação de botões mantida...
                    ...
                    // Se precisou clicar em CORRIGIR ONLINE, espera um pouco para a página processar
                    if (needsCorrectorClick) {
                        console.log("Aguardando brevemente após clique em 'CORRIGIR ONLINE'...");
                ...
                if (errorSpans.length === 0) {
                     ...
                     finalMessage = "Nenhum erro encontrado para correção."; finalType = "info";
                     correctionProcessRan = true;
                     if (correctionMode === 'advanced') hideAdvancedCorrectionSplash(); // Esconde splash se não achou erros
                }
                else {
                    console.log(`Encontrados ${errorSpans.length} spans de erro potenciais.`);
                    correctionProcessRan = true;
                    for (let i = 0; i < errorSpans.length; i++) {
                        // ... (TODA a lógica interna do loop FOR para processar cada span é MANTIDA EXATAMENTE IGUAL - V1.11) ...
                ...
                // Esconde o splash de correção ANTES de mostrar o prompt da IA
                 if (correctionMode === 'advanced') {
                     hideAdvancedCorrectionSplash();
                     await new Promise(r => setTimeout(r, 450)); // Espera animação de saída do splash
                 } else {
                     console.log("Modo Básico: Finalizando.");
                     await new Promise(r => setTimeout(r, 50));
                 }

                // --- Lógica da IA com Progresso Funcional ---
                if (correctionProcessRan && targetTextarea && document.body.contains(targetTextarea)) {
                     // Garante remover overlays de execuções anteriores
                     removeOverlay('bmAIReviewSplash');
                     removeOverlay('bmAILoadingSplash');
                     ...
                     if (wantAIReview) {
                         ...
                         aiStatusIntervalId = setInterval(() => {
                             ...
                             else { clearInterval(aiStatusIntervalId); aiStatusIntervalId = null;} // Para se o elemento sumir
                         ...
                         try {
                             ...
                             updateAIProgressBar(loadingUIData.progressBarElement, 20); // Progresso antes da chamada

                             const aiCorrectedText = await callPuterAI(currentText);
                             console.log("IA retornou correção.");

                              // Atualiza progresso e texto após chamada da IA (antes de limpar/digitar)
                             ...
                             if (aiStatusIntervalId) clearInterval(aiStatusIntervalId); aiStatusIntervalId = null; // Para intervalo aqui

                             await clearTextareaSimulated(targetTextarea); // <<< USA Select All + Backspace
                             updateAIProgressBar(loadingUIData.progressBarElement, 90); // Progresso após limpar

                             await typeTextFast(aiCorrectedText, targetTextarea); // Digitação rápida

                             // Finaliza: Barra 100%, última mensagem e pausa
                             ...
                             await new Promise(r => setTimeout(r, 700)); // Pausa maior para ver msg final
                 ...
                 showCustomAlert(finalMessage, finalType); // Mostra resultado final

            } catch (e) {
                 // ... (bloco catch mantido V1.11) ...
                 ...
                 // Garante esconder o splash de correção se um erro ocorreu antes da IA
                 hideAdvancedCorrectionSplash(); // Chama a função atualizada que usa removeOverlay
            } finally {
                 // ... (bloco finally mantido e reforçado V1.11) ...
                 ...
                 if (aiStatusIntervalId) clearInterval(aiStatusIntervalId); aiStatusIntervalId = null;
                 // Remove todos os overlays possíveis que podem ter ficado abertos
                 removeOverlay(loadingUIData?.overlayElement);
                 removeOverlay('bmAIReviewSplash');
                 removeOverlay('bmAILoadingSplash'); // Redundante talvez, mas seguro
                 removeOverlay('bmAdvCorrectionSplash'); // Remove splash de correção se ainda existir
                 // Remove alertas genéricos
                 removeOverlay('bmAlertOverlay');
