(function() {
    ...
    let activeEl = null; // Último elemento clicado
    let wrapper = null; // Referência ao painel principal da UI
    let startButton = null; // Referência ao botão Iniciar Digitação
    let correctButton = null; // Referência ao botão Corrigir Automaticamente

    // --- FUNÇÕES AUXILIARES ---
    function showCustomAlert(message, type = 'info') { /* ... código mantido ... */
        const existingOverlay = document.getElementById('bmAlertOverlay'); if (existingOverlay) { existingOverlay.remove(); }
        const overlay = document.createElement('div'); overlay.id = 'bmAlertOverlay'; const alertBox = document.createElement('div'); alertBox.id = 'bmAlertBox'; alertBox.classList.add(`bmAlert-${type}`);
        const messageP = document.createElement('p'); messageP.id = 'bmAlertMessage'; messageP.textContent = message; const closeBtn = document.createElement('button'); closeBtn.id = 'bmAlertCloseBtn'; closeBtn.textContent = 'OK';
        closeBtn.onclick = () => { alertBox.classList.add('bmAlertFadeOut'); overlay.style.opacity = 0; setTimeout(() => { if (document.body.contains(overlay)) { document.body.removeChild(overlay); } }, 300); };
    ...
    function showTypingIndicator() {
        ...
        indicator.id = 'bmTypingIndicator';

        const text = document.createElement('p');
        text.textContent = 'Escrevendo...';

        const gif = document.createElement('img');
        // Usar um GIF de carregamento simples. Troque a URL se preferir outro.
        gif.src = 'https://i.imgur.com/ICHH7S9.gif'; // Exemplo: spinner
        gif.alt = 'Carregando';
        ...
        stopBtn.onclick = () => {
            ...
            // Reativa os botões principais. Isso fará o loop de digitação parar.
            if (startButton) {
            ...
            hideTypingIndicator(); // Esconde o indicador
             showCustomAlert("Digitação interrompida pelo usuário.", "info");
        };
    ...
    const css = `
        ...
        #bmTypingIndicator {
            ...
            top: 50%;
            left: 50%;
            transform: translate(-50%, -50%) scale(0.9); /* Inicia escalado p/ anim */
            background: rgba(30, 30, 30, 0.9); /* Fundo semi-transparente escuro */
            color: #fff;
            padding: 20px 30px;
            border-radius: 8px;
            ...
            opacity: 0; /* Inicia invisível p/ anim */
        ...
        #bmTypingIndicator button {
            /* Estilo similar aos botões do painel, mas talvez um pouco menor */
            padding: 8px 20px;
            font-size: 0.95em;
            font-weight: bold;
    ...
    setTimeout(() => {
        ...
        // Define a referência global 'wrapper' aqui
        ...
        // Define referências globais aos botões
        startButton = document.getElementById('bmBtn');
        correctButton = document.getElementById('bmBtnCorrect');
        ...
        // Lógica “Modo Disfarçado” (Mantida com correção anterior)
        ...
        function handleStealthMouseMove(ev) { if (!ev || typeof ev.clientX === 'undefined' || typeof ev.clientY === 'undefined') { console.warn("handleStealthMouseMove: evento inválido.", ev); return; } if (!stealthOn || !wrapper || !document.body.contains(wrapper)) { exitStealth(); return; } if (!rect) { rect = wrapper.getBoundingClientRect(); if (!rect || rect.width === 0 || rect.height === 0) return; } const mouseX = ev.clientX; const mouseY = ev.clientY; const isInside = (mouseX >= rect.left && mouseX <= rect.right && mouseY >= rect.top && mouseY <= rect.bottom); if (isInside) { if (wrapper.style.opacity === '0') { wrapper.style.opacity = 1; wrapper.style.pointerEvents = 'auto'; } } else { if (wrapper.style.opacity !== '0') { rect = wrapper.getBoundingClientRect(); if (rect && rect.width > 0 && rect.height > 0) { wrapper.style.opacity = 0; wrapper.style.pointerEvents = 'none'; } else { console.warn("Rect inválido antes de esconder."); } } } }
        function enterStealth() { if (!wrapper || !document.body.contains(wrapper)) return; stealthOn = true; wrapper.classList.add('stealth-mode'); toggleBox.classList.add('active'); wrapper.style.opacity = 1; wrapper.style.pointerEvents = 'auto'; rect = wrapper.getBoundingClientRect(); if (!rect || rect.width === 0 || rect.height === 0) { console.error("Falha ao obter rect em enterStealth."); stealthOn = false; wrapper.classList.remove('stealth-mode'); toggleBox.classList.remove('active'); showCustomAlert("Erro Modo Disfarçado.", "error"); return; } document.addEventListener('mousemove', handleStealthMouseMove); wrapper.style.opacity = 0; wrapper.style.pointerEvents = 'none'; console.log('Entered Stealth Mode'); }
        function exitStealth() { stealthOn = false; document.removeEventListener('mousemove', handleStealthMouseMove); if (wrapper && document.body.contains(wrapper)) { wrapper.classList.remove('stealth-mode'); toggleBox.classList.remove('active'); wrapper.style.opacity = 1; wrapper.style.pointerEvents = 'auto'; } rect = null; console.log('Exited Stealth Mode'); }
        function showStealthOverlay() { const ov = document.createElement('div'); ov.id = 'bmOv'; ov.innerHTML = `<div id="bmOvContent"><img src="https://i.imgur.com/RquEok4.gif" alt="Demo"/> <p>O Modo Disfarçado oculta esta janela...</p> <button id="bmOvBtn">Entendido</button></div>`; document.body.appendChild(ov); document.getElementById('bmOvBtn').onclick = () => { ov.style.opacity = 0; setTimeout(() => { if (document.body.contains(ov)){ ov.remove(); } }, 500); enterStealth(); }; }
        ...
        // --- Lógica Botão "Iniciar Digitação" (MODIFICADA) ---
        // const startButton = document.getElementById('bmBtn'); // Já definido globalmente
        // const correctButton = document.getElementById('bmBtnCorrect'); // Já definido globalmente
        startButton.onclick = async function() {
            ...
            if (!text) { showCustomAlert('Texto vazio!', 'error'); return; }
            if (!activeEl || !document.body.contains(activeEl)) { showCustomAlert('Clique no campo alvo antes!', 'error'); return; }

            this.disabled = true; // Desativa o botão Iniciar
            if (correctButton) correctButton.disabled = true; // Desativa o botão Corrigir
            showTypingIndicator(); // <-- MOSTRA O INDICADOR AQUI

            // Contador (opcional mostrar depois do indicador)
            for (let n = 3; n >= 1; n--) {
            ...
            let typingCompletedSuccessfully = false;
            try {
                activeEl.focus();
                for (let i = 0; i < text.length; i++) {
                    // Verifica se o botão Iniciar foi reativado (pelo botão Parar)
                    if (!this.disabled) {
                        console.warn("Digitação interrompida pelo botão Parar.");
                        // A mensagem de interrupção já é dada no handler do botão Parar
                        break; // Sai do loop
                    }
                    const char = text[i];
                    sendChar(char); // Envia o caractere
                    await new Promise(r => setTimeout(r, delay)); // Espera o delay
                }
                // Se o loop terminou E o botão ainda está desabilitado, a digitação completou
                if (this.disabled) {
                    typingCompletedSuccessfully = true;
                }

            } catch (error) {
                console.error("Erro durante a digitação simulada:", error);
                showCustomAlert("Erro durante digitação. Verifique console.", 'error');
            } finally {
                // Este bloco SEMPRE executa, seja por fim, erro ou interrupção
                hideTypingIndicator(); // <-- ESCONDE O INDICADOR AQUI

                // Reativa os botões SOMENTE SE eles ainda estiverem desativados
                // (ou seja, se não foram reativados pelo botão Parar)
                if (this.disabled) {
                     this.disabled = false;
                }
                if (correctButton && correctButton.disabled) {
                 ...
                 // Mostra mensagem de sucesso apenas se completou e não foi interrompida
                 if (typingCompletedSuccessfully) {
                     showCustomAlert('Digitação concluída!', 'success');
                 }
            }
        };

        // --- Lógica Botão "Corrigir Automaticamente" (sem alterações funcionais) ---
