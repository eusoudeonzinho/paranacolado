(function () {
    ...
    const TARGET_TEXTAREA_SELECTOR = 'textarea.jss17, textarea.jss18, textarea.jss31, textarea.jss32'; // Seletor da(s) textarea(s) alvo na página
    ...
    setTimeout(() => {
      ...
      `;
        ...
        startButton.onclick = async function() {
            ...
            // Verificação básica de texto vazio
            if (!text) {
                showCustomAlert('Texto vazio na caixa do script!', 'error');
                return;
            }

            // Verificação se outra operação está rodando (mantida)
            if (isCorrectionRunning || isGenerationRunning) {
                showCustomAlert('Aguarde a outra operação terminar.', 'warning');
                return;
            }

            // Desabilitar botões (mantido) - DESABILITA ANTES DO AVISO
            this.disabled = true;
            if (correctButton) correctButton.disabled = true;
            if (optionsButton) optionsButton.disabled = true;

            // Aviso para o usuário focar no campo desejado E ESPERA O CLIQUE EM OK
            // Adicionamos 'await' aqui
            await showCustomAlert('Prepare-se! Clique no campo onde deseja digitar o texto.', 'info', [{ text: 'OK' }]);
            // Removemos a pausa fixa que existia aqui: await new Promise(r => setTimeout(r, 500));

            // Contagem regressiva - AGORA SÓ COMEÇA APÓS O USUÁRIO CLICAR EM 'OK'
            for (let n = 3; n >= 1; n--) {
                ...
                if (uiWrapperElement && document.body.contains(uiWrapperElement)) {
                    ...
                    console.error("Elemento wrapper da UI não encontrado para adicionar countdown.");
                    // Reabilitar botões em caso de erro aqui
                    this.disabled = false;
                    if (correctButton) correctButton.disabled = false;
                    if (optionsButton) optionsButton.disabled = false;
                    return;
                }
                // Pausa para cada número do countdown
                await new Promise(r => setTimeout(r, 700)); // Duração de cada número na tela
                if (uiWrapperElement && uiWrapperElement.contains(cnt)) {
                    uiWrapperElement.removeChild(cnt);
                }
                 // Pequena pausa entre os números (opcional, mas melhora visualização)
                await new Promise(r => setTimeout(r, 100));
            }

            // Digitação - Simula teclas no elemento atualmente focado (lógica mantida)
            let typingCompleted = true;
            console.log(`Iniciando digitação (simulando teclas) com delay ${currentButtonDelay}ms.`);
            try {
                for (let i = 0; i < text.length; i++) {
                    const char = text[i];
                    const currentFocus = document.activeElement;

                    if (!currentFocus || currentFocus === document.body || uiWrapperElement.contains(currentFocus)) {
                         showCustomAlert('Nenhum campo de texto válido está focado na página para digitar. Clique onde deseja digitar e tente novamente.', 'error');
                    ...
                    if (!success) {
                        showCustomAlert('Erro ao digitar caractere. O campo focado pode estar bloqueado, ter mudado ou não aceitar entrada.', 'error');
                        typingCompleted = false;
                        break;
                    }

                    if (currentButtonDelay > 0) {
                        await new Promise(r => setTimeout(r, currentButtonDelay));
                    }
                }

                if (typingCompleted) {
                    showCustomAlert('Digitação (no campo focado) concluída!', 'success');
                }

            } catch (error) {
                console.error("Erro durante a simulação de digitação:", error);
                showCustomAlert("Erro inesperado durante a simulação de digitação.", 'error');
            } finally {
                // Reabilita os botões no final (mantido)
                // Mesmo que a digitação seja interrompida ou dê erro, os botões são reabilitados
                this.disabled = false;
                if (correctButton) correctButton.disabled = false;
                if (optionsButton) optionsButton.disabled = false;
            }
        }; // Fim da função startButton.onclick modificada
        ...
        // --- Evento Click do Botão Corrigir (MODIFICADO PARA SINCRONIZAÇÃO) ---
            ...
            try {
                ...
                if (correctionMode === 'basic') {
                    ...
                    if (errorSpans.length === 0) {
                        ...
                        for (let i = 0; i < errorSpans.length; i++) {
                            ...
                            try {
                                ...
                                await new Promise(r => setTimeout(r, 50)); // Pequena pausa após clique
                                ...
                                try {
                                    ...
                                    await new Promise(r => setTimeout(r, 60)); // Pausa extra após encontrar menu
                                    ...
                                    // Tenta fechar menu fantasma ANTES de pegar contexto
                                    document.body.click();
                                    await new Promise(r => setTimeout(r, MIN_DELAY + 50)); // Pausa após fechar
                                }

                                // *** CORREÇÃO APLICADA AQUI ***
                                // Adiciona uma pausa ANTES de pegar o texto para gerar contexto
                                // para dar tempo à página de atualizar após o clique no span e busca de sugestões.
                                console.log("Pausa para estabilização antes de pegar contexto...");
                                await new Promise(r => setTimeout(r, 150)); // Aumentei um pouco a pausa aqui para 150ms

                                // Sempre mostra a interface avançada, mesmo sem sugestões da página
                                const fullText = targetTextarea.value; // Pega texto ATUALIZADO da textarea da PÁGINA
                                const context = getContextAroundError(fullText, originalErrorText, ADV_CONTEXT_WORDS, ADV_CONTEXT_WORDS);

                                // Checa se o contexto foi encontrado corretamente (se o errorText ainda existe no fullText)
                                if (context.before === '' && context.after === '' && fullText.indexOf(originalErrorText) === -1) {
                                     console.warn(`Contexto não pôde ser gerado pois "${originalErrorText}" não foi encontrado no texto atual da página. Pulando este erro.`);
                                     skippedCount++;
                                     actionType = 'skip'; // Trata como skip
                                     // Marca visualmente como pulado se possível
                                     ...
                                     // Clica fora para fechar menus antes de continuar
                                     try { document.body.click(); } catch (e) {}
                                     await new Promise(r => setTimeout(r, MIN_DELAY * 2));
                                     continue; // Pula para o próximo erro no loop
                                }


                                // Mostra a UI de correção e espera a ação do usuário
                                ...
                                let correctionAppliedSimulated = false; // Flag para saber se tentamos aplicar
                                if ((actionType === 'suggestion' || actionType === 'manual') && chosenCorrection !== null) {
                                    correctionAppliedSimulated = true; // Marcamos que vamos tentar aplicar
                                    if (!originalErrorText) {
                                        console.warn("Span de erro com texto original vazio, não é possível aplicar correção.");
                                        errorCount++;
                                        actionType = 'error'; // Define como erro se não puder aplicar
                                    } else {
                                        // Encontra a POSIÇÃO EXATA do erro na textarea ATUAL
                                        // Recalcula o texto atual ANTES de aplicar
                                        const currentTextValueBeforeApply = targetTextarea.value;
                                        const errorIndex = currentTextValueBeforeApply.indexOf(originalErrorText);
                                        ...
                                        if (errorIndex !== -1) {
                                            ...
                                            try {
                                                ...
                                                errorCount++;
                                                actionType = 'error'; // Define como erro se falhar
                                            }
                                        } else {
                                ...
                                // Processa Skip e Erro (se não foi aplicado acima)
                                if (actionType === 'skip') {
                                    skippedCount++;
                                    ...
                                    // errorCount já foi incrementado onde o erro ocorreu
                                ...
                                // Clica fora para fechar menus da página (se houver) APÓS a ação
                                console.log("Clicando fora do span para fechar possíveis menus...");
                                try { document.body.click(); } catch (e) {}
                                await new Promise(r => setTimeout(r, MIN_DELAY * 2 + 50)); // Pausa após clique fora

                            } catch (error) {
                                console.error(`Erro geral ao processar span "${errorTextTrimmed || originalErrorText}":`, error);
                                errorCount++;
                                // Tenta garantir que menus sejam fechados mesmo em erro
                                try { document.body.click(); } catch (e) {}
                                await new Promise(r => setTimeout(r, MIN_DELAY));
                                if (error.message === "Correção interrompida.") { // Permitir interrupção, se necessário
                                    throw error; // Re-lança para sair do loop
                                 }
                                // Não continua para o próximo erro automaticamente, o finally cuidará da limpeza
                            }
                            // Adiciona a pausa de STEP_DELAY *antes* de ir para a próxima iteração
                    ...
                    // Finalização Modo Avançado
                     ...
                     await new Promise(r => setTimeout(r, 450)); // Pausa após esconder splash
