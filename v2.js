(function() {
  ...
  setTimeout(() => {
    ...
    // variáveis de stealth
    let stealthState = false,
        stealthSeen  = false,
        rect;

    // envia tecla para o activeEl
    function sendChar(c) {
      if (!activeEl) return;
    ...
    function applyNormal() {
      ...
      wrapper.querySelectorAll('button.bmBackBtn, button#bmBtn').forEach(b=>{
        b.style.background='#8A2BE2'; b.style.color='#fff';
      });
    ...
    function showMenu() {
      ...
      document
        .getElementById('bmMenuBtnCorrection')
        .onclick = showCorrection;
      document
        .getElementById('bmMenuBtnPaste')
        .onclick = showPasteUI;
    }

    // tela Correção Online (vazia por enquanto)
    function showCorrection() {
      ...
      setTimeout(()=>content.classList.add('content-show'),50);
      document
        .getElementById('bmBack1')
        .onclick = showMenu;
    }

    // tela Colar Textos (interface existente)
